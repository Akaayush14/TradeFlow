...
class ChatRepoImpl : ChatRepo {
    ...
    override fun getMessages(
        ...
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                // Sort messages by timestamp and trigger onMessagesUpdate
    ...
    override fun deleteMessage(
        ...
        val user = getCurrentUser() ?: return callback(false, "Not logged in")
        val otherId = if (message.senderId == user.uid) message.receiverId else message.senderId
        val chatId = generateChatId(user.uid, otherId)

        messagesRef.child(chatId).child(message.messageId).removeValue()
            .addOnSuccessListener {
                callback(true, "Message deleted")
            }
            .addOnFailureListener {
                callback(false, it.message ?: "Failed to delete message")
            }
    }
    ...
    override fun getChatSummaries(
        callback: (Boolean, String, List<ChatModel>?) -> Unit
    ) {
        val user = getCurrentUser() ?: return callback(false, "Not logged in", null)
        messagesRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val summaries = mutableListOf<ChatModel>()
                snapshot.children.forEach { chatSnapshot ->
                    val chatId = chatSnapshot.key ?: return@forEach
                    val parts = chatId.split("_")
